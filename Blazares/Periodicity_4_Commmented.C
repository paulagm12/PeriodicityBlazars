...
// Define constant value w0 as TwoPi from TMath library
const Double_t w0 = TMath::TwoPi();

// Function to define Wavelet in Frequency Domain
...
void CWT_mw(const char *FileName)
{
    ...
    Double_t Period = DFtime.back() - DFtime.front();
    for (Int_t i = 0; i < n; i++)
    {
        zeros[i] = 0;
    }

    // Creating a graph to represent the loaded data
    TGraph *FMed = new TGraph(DFtime.size(), DFtime.data(), DFflux.data());

    // Initializing arrays for FFT and Wavelet transform
    ...
    // Performing FFT on the loaded data
    ...
    // Calculation of parameters for CWT and performing the transform
    ...
    // Calculating frequency and time bins
    ...
    // Creating a 2D histogram for the CWT representation
    TH2D *aHist2 = new TH2D("hcwt", "CWT", n, DFtime.front(), DFtime.back(), TNumber, Tbins);

    // Loop for Continuous Wavelet Transform
    for (Int_t j = 1; j <= TNumber; j++)
    {
        // Generating the wavelet function for the current frequency
        for (Int_t i = 0; i < n; i++)
        {
            mfftre[i] = WaveletFD(i, Period * F[j], 1);
        }

        // Applying the wavelet transform to the FFT data
        for (Int_t i = 0; i < n; i++)
        {   
        ...
        // Performing backward FFT to get the CWT
        ...
        // Calculating the CWT projection
        Proy[TNumber - j] = 0;
    ...
    // Creating a canvas and displaying the graphs/histograms
    ...
    Double_t maxPeriod = 1.0 / TCbins[0];
    std::cout << "Maximum period: " << maxPeriod << std::endl;

    // Freeing memory allocated for arrays
